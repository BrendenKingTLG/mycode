...
def movielookup(mykey, searchstring, type="", year=""):
    """Interactions with OMDB API
       mykey = omdb api key
       searchstring = string to search for"""
    try:
        # begin constructing API
        api = f"{OMDBURL}apikey={mykey}&s={searchstring}&type={type}&y={year}"
    ...
    while True:
        ...
        while answer == "":
            ...
            ** Returned data will be written into the local database **
            ...
            3) Search for movies by name and year
            99) Exit""")
        ...
        if answer in ["1", "2", "3"]:
            ...
            if answer == "1":
                ...
                movie_type = input("enter a search type: ")
                resp = movielookup(mykey, searchstring, movie_type)          # restart the while loop
            elif answer == "3":
                movie_year = input("enter a search year: ")
                resp = movielookup(mykey, searchstring, year = movie_year)  
            if resp:
                # display the results
